...
from projects.videochat.util import *
import os
import shutil
import re
import openai
...
class ProcessOCR:
    ...
    def merge(self):
        ...
        dense_with_ocr, ocr_subtitle = self.find_text_in_dense(self.features, video_width, video_height)
    ...
    def chatgpt_check_subtitle(self, question):
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "用户将输入一段字幕，其中相邻的几句可能是同一句话，但是由于识别错误导致了重复。你需要将其中识别错误的语句删去。\n\n例如用户输入：```\nSecond 61: 你这瓜要是熟我肯定要啊\nSecond 62: 你这瓜要是熟我背定要啊\n```\n你应该删去```Second 62: 你这瓜要是熟我背定要啊```，保留```Second 61: 你这瓜要是熟我肯定要啊```，所以你的输出应该是：\n```Second 61: 你这瓜要是熟我肯定要啊```\n注意：你只需要给出修改后的字幕，不要输出任何思考过程。下面是用户输入："
                },
                {
                    "role": "user",
                    "content": question
    ...
    def merge_ocr_with_subtitle(self, data):
        ...
        question = '"merged_subtitle": {},\n"ocr_subtitle": {}'.format(data['merged_subtitle'], data['ocr_subtitle'])
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {
                    "role": "system",
                    "content": "用户将输入一段语音识别字幕和OCR识别字幕，它们都有识别错误的地方，你需要将它们相互校对，形成最符合逻辑的字幕。\n你可以删除几种语音识别字幕中的错误：\n1. 中文对话中突然出现英文\n2. 突然出现没有逻辑的语气词，如“啊”“吧”等\n3. 不符合语境的词语"
                },
                {
                    "role": "user",
                    "content": question
