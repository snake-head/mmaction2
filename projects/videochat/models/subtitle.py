...
class RequestApi(object):
    ...
    def result2text(self, return_list=False):
        ...
        for item in lattice:
            ...
            sentence['begin'] = int(int(json.loads(item['json_1best'])['st']['bg']) / 1000)
            sentence['end'] = int(int(json.loads(item['json_1best'])['st']['ed']) / 1000)
            sentence_list.append(sentence)
        result = ''
        for sentence in sentence_list:
            result += 'Second {} to Second {}, Speaker {}: {}\n'.format(sentence['begin'], sentence['end'],
    ...
    def merge_whisper_and_xunfei(self):
        ...
        for whisper_item in whisper:
            ...
            if '\u4e00' <= whisper_item['text'][0] <= '\u9fff':
                find_item = self.find_match_subtitle(whisper_item['begin'], xunfei)
                if not find_item:
